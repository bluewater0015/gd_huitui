...
export default class Execute extends Component{
	...
	fetchData(){
		...
		return fetch(url,{
			...
			console.log('distributeImages',data.distributeImages);
	...
	confirmEvent() {
		...
		//console.log('转发中的files',this.state.distributeImages);
		...
		if(this.state.form.files) {
        	for(let k = 0; k < this.state.form.files.length; k++){
        		if(typeof this.state.form.files[k] === 'string'){
        			disArr.push(this.state.form.files[k]);
        		}else{
        			FileArr.push(this.state.form.files[k]);
        		}
        	}
		}
		...
		let param ={
			...
			distributeImages: disArr
		}
		let formData = new FormData();
		
		FileArr.forEach(f =>{
			console.log('resultImages-f',f);
	...
	getfiles(acceptedFiles) {
		...
		//console.log('11acceptedFiles',acceptedFiles);
	...
	//点击派发信息图片中的每张
	createdPicEvent(e,index) {
		//console.log('index',index);
		this.setState({
			blackShade: true,
			src: this.state.createdPic[index],
		})
	}
	//黑色遮罩层
	blackShadeShowModel() {
		return(
			<div className="blackShade center" 
			onClick={()=>this.blackShadeEvent()} >
				<div>
			...
			<div className="execute">
							...
							this.state.createImages.map((item,index)=>{
								return(
									<li className="img_item float-left" key={index}>
										<img src={item} alt="" onClick={ (e)=>this.createdPicEvent(e,index) }/>
									</li>
								)
							})
						}
				...
				<div className="execute_message">
					...
					<div className="margin-bottom">
						...
						distributeImages={this.state.distributeImages}
						getSrc={(src)=>this.getSrcEvent(src)} 
						getDelete={ this.deleteImages.bind(this)} />
					</div>
				</div>
				<div className="margin-bottom padding border-bottom">
					<div className="padding-bottom flex"> 
						<p>执行信息</p>
						<p className="color_red font_weight">（若要转发工单，请勿填写执行信息）</p>
					</div>
				</div>
				<div className="padding">
					...
					<div className="flex">
						...
						getSrc={(src)=>this.getEquipSrcEvent(src) }/>
					</div>
					...
					<div className="flex">
						...
						getSrc={(src)=>this.getHealthSrcEvent(src) } />
					</div>
					...
					<div className="flex">
						...
						getfiles={ this.getCompeteFiles.bind(this)}
						getSrc={(src)=>this.getCompeteSrcEvent(src) }/>
					</div>
				</div>
				...
				{
					this.state.blackShade ? this.blackShadeShowModel(): null
				}
				{
