...
export default class Complete extends Component{
	...
	fetchData(){
		...
		return fetch(url,{
			...
			this.setState({
				...
				//派发图片
				distributePic: data.distributeImages,
				//设备图片
				equipmentPic: data.workOrderExecuteDTO? data.workOrderExecuteDTO.equipmentImages:[],
				//卫生图片
				healthPic: data.workOrderExecuteDTO? data.workOrderExecuteDTO.healthImages:[],
				//竞品图片
				compeletePic: data.workOrderExecuteDTO? data.workOrderExecuteDTO.awardPriceImages:[],
	...
	//点击派发信息图片中的每张
	distributePicEvent(index) {
		this.setState({
			blackShade: true,
			src: this.state.distributePic[index],
		})
	}
	//点击设备图片中的每张
	equipmentPicEvent(index) {
		this.setState({
			blackShade: true,
			src: this.state.equipmentPic[index],
		})
	}
	//点击卫生图片中的每张
	healthPicEvent(index) {
		this.setState({
			blackShade: true,
			src: this.state.healthPic[index],
		})
	}
	//点击竞品图片中的每张
	compeletePicEvent(index) {
		this.setState({
			blackShade: true,
			src: this.state.compeletePic[index],
		})
	}
	//遮罩层事件
	blackShadeEvent() {
									...
									this.state.createImages.map((item,index)=>{
										return(
											<li className="img_item float-left" key={index}>
												<img src={item} alt="" onClick={ ()=>this.createdPicEvent(index) }/>
											</li>
										)
									})
								}
							...
							<ul className="completeImages clearfix">
								{
									this.state.distributeImages.map((item,index)=>{
										return(
											<li className="img_item float-left" key={index}>
												<img src={item} alt="" onClick={ ()=>this.distributePicEvent(index) }/>
											</li>
										)
									})
								}
								...
								<ul className="equipmentImages clearfix">
									{
										this.state.equipmentImages.map((item,index)=>{
											return(
												<li className="img_item float-left" key={index}>
													<img src={item} alt="" onClick={ ()=>this.equipmentPicEvent(index) }/>
												</li>
											)
										})
									}
								...
								<ul className="healthImages clearfix">
									{
										this.state.healthImages.map((item,index)=>{
											return(
												<li className="img_item float-left" key={index}>
													<img src={item} alt="" onClick={ ()=>this.healthPicEvent(index) }/>
												</li>
											)
										})
									}
								...
								<ul className="awardPriceImages clearfix">
									{
										this.state.awardPriceImages.map((item,index)=>{
											return(
												<li className="img_item float-left" key={index}>
													<img src={item} alt="" onClick={ ()=>this.compeletePicEvent(index) }/>
												</li>
											)
										})
									}
